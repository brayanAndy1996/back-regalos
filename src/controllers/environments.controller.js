...
const createEnvironment = async (req, res = response) => {
    try {
        const { name, description, entryTime, entryDeadline, departureTime, area, location, tutor } = req.body;
        const data = {
            name: name?.toUpperCase(),
            description: description?.toUpperCase(),
            entryTime: assingADateToTime(entryTime),
            entryDeadline: assingADateToTime(entryDeadline),
...
const updateEnvironment = async (req, res = response) => {
    try {
        ...
        const { description, entryTime, entryDeadline, departureTime, area, location, tutor } = req.body;
        const data = {
            description: description?.toUpperCase(),
            entryTime: assingADateToTime(entryTime),
            entryDeadline: assingADateToTime(entryDeadline),
        ...
        const environmentUpdate = await Environment.findByIdAndUpdate(id, data, { new: true });
